...
int main(int argc, char *argv[]) {
  ...
  /*     CHECK ARGUMENTS     */
  /***************************/
  if (input.isub_out < 1 || input.nsub < input.isub_out) {
  ...
  if (input.silent == OFF) {
    ...
    if (input.mode == SEQUENT) {
      printf(" Alignment mode : sequential\n\n");
    }
    if (input.mode == FWandRV) {
      printf(" Alignment mode : forward/reverse mixed\n\n");
    }
  ...
  if (input.silent == OFF) {
    printf(" Finished successfully ( %4.3f sec )\n\n",
...
void usage(void) {
  printf(
      ...
      "  -t  Integer    selection score ([0]:MICAN, 1:TMscore, 2:Dali-Z)\n"
      ...
      FAST_GHcutnum, DEF_nsub, DEF_GHcutnum, DEF_lmin);
  exit(1);
}
